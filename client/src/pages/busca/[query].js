...
export async function getServerSideProps(context) {
  ...
  let itens_page = 20;
  let QUERY_POST_BUSCA;
  if (!page) {
    page = 1;
  }
  if (page == 1) {
    QUERY_POST_BUSCA = gql`
    {
      publicacoes(where:{_or:[{titulo_contains:"${query}"}, {resumo_contains:"${query}"}, {tags_contains: "${query}"}]}, 
      start:0 ,limit:${itens_page}, sort: "id:desc") {
        id
        titulo
        resumo
        slug
      ...
      publicacoesConnection(where:{_or:[{titulo_contains:"${query}"}, {resumo_contains:"${query}"}, {tags_contains: "${query}"}]}){
        aggregate{
          count
        }
      }
  ...
  `;
    ...
    {
      publicacoes(where:{_or:[{titulo_contains:"${query}"}, {resumo_contains:"${query}"}, {tags_contains: "${query}"}]}, 
      start:${Number(start)} ,limit:${itens_page}, sort: "id:desc") {
        id
        titulo
        resumo
        slug
  ...
  const posts_busca = await apolloClient.query({ query: QUERY_POST_BUSCA });
